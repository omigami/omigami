...
def test_match_spectra_from_path_with_2_spectra(
    ...
    assert (
        result[0].sort_values(by=["score"], ascending=False).index[0]
        == "CCMSLIB00000001547"
    )
    assert (
        result[1].sort_values(by=["score"], ascending=False).index[0]
        == "CCMSLIB00000001548"
    )
...
@pytest.mark.internet_connection
@pytest.mark.skip(reason="Requires a valid token")
def test_match_spectra_from_path_with_small(
    small_mgf_path, ms2deepscore_client
):
    result = ms2deepscore_client.match_spectra_from_path(
